...
async function importSample(tokens: DesignTokens): Promise<number> {
  ...
  function buildFileUpload(c: ComponentNode, out: PendingProp[], state: string) {
    ...
    c.appendChild(title)
    const hint = txt('SVG, PNG or JPG (max. 800×400px)', { size: 12, sizeVar: sizeXs, colorP: p.textTertiary })
    c.appendChild(hint)
  ...
  function buildRating(c: ComponentNode, out: PendingProp[], interactive: boolean) {
    ...
    c.appendChild(stars)
    if (interactive) {
      const count = txt('4.0 · 128 reviews', { size: 12, sizeVar: sizeXs, colorP: p.textTertiary })
      count.name = 'count'
      c.appendChild(count)
