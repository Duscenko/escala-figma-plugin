...
| `typography` | **Typography** | `size/*`, `weight/*`, `font-family-body`, `font-family-display`, `line-height/*`, `letter-spacing/*`, plus v6 `role/{key}/{size,weight,family}` aliases (+ one text style per size **and** one per semantic role, bound to those variables). Each family variable contains the concrete Figma family name from the current payload; an existing `family` / `heading-family` is renamed in place where possible, preserving old bindings. |
